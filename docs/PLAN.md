...
| **3b** | Interfaz de práctica de test | **NÚCLEO** <!-- P-001, aprobada 2026-08-10 --> | `ui/` — servir una pregunta del banco, registrar la opción elegida y mostrar la explicación citada del motor. **Frontera única: la API HTTP.** `ui/` va en `[tool.gate].excluido` y `tdd_prohibido` | Entra en `make done MILESTONE=3`. Test de contrato contra el snapshot de OpenAPI, y humo e2e: `compose up`, responder una pregunta, hay cita | Contrato (snapshot OpenAPI) y e2e de humo. **Sin TDD ni cobertura por función**: es presentación, no dominio (ADR-019) | ninguna propia. No introduce meta nueva ni toca un umbral | 8-12 | 0 | **[S]**, al cierre de la 3 |
| **6** | BKT y selector de bloque | **AMPLIACIÓN** | `domain/knowledge.py` (BKT en NumPy, 4 parámetros) + `domain/selector.py`, ambos dominio puro. Simulador determinista con semilla como banco de pruebas del selector. **La selección adaptativa es lo único que queda aquí**: servir preguntas y explicarlas es la fase 3b | `pytest tests/property/test_knowledge.py tests/property/test_selector.py` → exit 0. **Timebox duro: 3 días** | TDD completo + Hypothesis: monotonía tras acierto, cotas [0,1], el selector nunca repite una pregunta dominada, respeta el tamaño solicitado | `G-BKT-PROP` (**nunca bloquea el núcleo**) | 12-18 | 0 | **[P]**, opcional |

<!-- P-001 · aprobada por Samuel el 2026-08-10 · aplicada por el agente con su autorización explícita.
     Los endpoints /session/* y la interfaz de práctica salen de la fase 6 y pasan al núcleo como
     fase 3b, obligatoria para `make done MILESTONE=3`. El BKT se queda en la 6 como ampliación.
     Ninguna meta nueva, ningún umbral tocado. Motivo y alternativas descartadas en
     docs/adr/019-frontera-motor-interfaz.md y en la propuesta P-001 de docs/PARA-SAMUEL.md. -->
