...
def x_decidir__mutmut_orig(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_decidir__mutmut_1(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_decidir__mutmut_2(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_decidir__mutmut_3(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_decidir__mutmut_4(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_decidir__mutmut_5(veredicto: Veredicto, *, reintentos_hechos: int, hay_fuentes: bool) -> Salida:
    """La decisión de un solo paso. Todo lo que la determina son sus tres argumentos.
...
def x_resolver_curso__mutmut_orig(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_1(intentos: Sequence[Veredicto], *, hay_fuentes: bool = False) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_2(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_3(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_4(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_5(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_6(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(None, reintentos_hechos=hechos, hay_fuentes=hay_fuentes)
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_7(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(veredicto, reintentos_hechos=None, hay_fuentes=hay_fuentes)
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_8(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(veredicto, reintentos_hechos=hechos, hay_fuentes=None)
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_9(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(reintentos_hechos=hechos, hay_fuentes=hay_fuentes)
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_10(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(veredicto, hay_fuentes=hay_fuentes)
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_11(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
    ...
    for hechos, veredicto in enumerate(intentos):
        ultimo = veredicto
        salida = decidir(veredicto, reintentos_hechos=hechos, )
        if salida is Salida.RESPONDER:
            return Curso(salida=salida, reintentos=hechos, refs=veredicto.refs)
        if salida is Salida.ABSTENERSE:
            return Curso(salida=salida, reintentos=hechos, motivo=veredicto.motivo)
...
def x_resolver_curso__mutmut_12(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_13(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_14(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_15(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_16(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_17(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_18(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_19(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_20(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_21(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_22(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_23(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_24(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_25(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_26(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_27(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_28(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_29(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_30(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_31(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_32(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_33(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_34(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_35(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_36(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_37(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
...
def x_resolver_curso__mutmut_38(intentos: Sequence[Veredicto], *, hay_fuentes: bool = True) -> Curso:
    """El curso entero a partir de los veredictos de cada borrador.
